...
class ManualGobbletPolicy:
    ...
    def __call__(self, observation, agent):
        ...
        while True:
            ...
            """ FIND PLACED PIECES """
            placed_pieces = env.unwrapped.board.squares[
                env.unwrapped.board.squares.nonzero()
            ]
            placed_pieces_agent = [
                a for a in placed_pieces if np.sign(a) == agent_multiplier
            ]
                ...
                if not picked_up:
                    if (
                        ...
                        cycle_choices = np.unique(
                            [(p + 1) // 2 for p in unplaced]
                        )  # Transform [1,2,3,4,5,6] to [1,2,3)

                        if len(cycle_choices) > 0:
                            piece_size = cycle_choices[
                                (np.amax(cycle_choices) - (piece_cycle + 1))
                                % len(cycle_choices)
                            ]
                        # else:
                        #   piece_size = -1
                        ...
                        if (
                            piece_size * 2
                        ) - 1 in unplaced:  # Check if the first of this piece size is available
                            piece = piece_size * 2 - 1
                        else:
                            piece = (
                                piece_size * 2
                            )  # Otherwise choose the second of this piece size
                    else:
            ...
            if piece != -1:
                ...
                action_prev = env.unwrapped.board.get_action(
                    pos, piece_size, env.agents.index(agent)
                )
            ...
            if action_prev != -1:
                if not env.unwrapped.board.is_legal(
                    action_prev, env.agents.index(agent)
                ):  # If this action is illegal
                    action_prev = -1
                else:
                    env.unwrapped.board.squares_preview[
                        pos + 9 * (piece_size - 1)
                    ] = agent_multiplier  # Preview this position
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if flatboard[pos] in placed_pieces_agent and not picked_up:
                    ...
                    if piece_size_on_board >= piece_size_selected:
                        ...
                        if not all(
                            observation["action_mask"][9 * (piece - 1) : 9 * piece] == 0
                        ):
                            picked_up = True
                            picked_up_pos = pos
                            ...
                            index = np.where(
                                env.unwrapped.board.squares == piece_to_pick_up
                            )[0][0]
                            ...
                            observation["action_mask"][
                                pos + 9 * (piece - 1)
                            ] = 0  # TODO: check if this is already zero
                            observation["action_mask"][
                                : 9 * (piece - 1)
                            ] = 0  # Zero out all the possible actions
